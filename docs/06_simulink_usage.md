...
The C++ controller inside pid\_mex is stateful. It owns:
...
params.deriv\_filter\_hz = 1.3029/(2\*pi);
...
From this point on, every pid\_mex('step', ...) call uses this same
controller instance.
...
function u = controller\_100Hz(ref, y\_meas)

% Call MEX from MATLAB execution (not codegen)

coder.extrinsic('pid\_mex');
...
u = pid\_mex('step', ref, y\_meas, dt\_ctrl);
...
blocks.  
That would fail here, because pid\_mex is a precompiled MEX binary.

coder.extrinsic('pid\_mex') tells Simulink:
...
* Simulink executes controller\_100Hz every 10 ms
* Each call advances the C++ PID by exactly one control step
* dt\_ctrl = 0.01 matches the actual sample time
